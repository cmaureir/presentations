...
\begin{itemize}
	...
	\item Cada \blue{nodo} representa una region en un espacio de dos dimensiones.
...
\begin{itemize}
	\item En un \blue{cuerpo} particular.
	 \begin{itemize}
	 	\item Recorrer los nodos del arbol, partiendo de la raíz.
	 \end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Ajustando \blue{$\theta$}, podemos cambiar la \green{velocidad} y la \green{presición} de la simulacion.
...
\begin{enumerate}
	...
	\item<2-> \red{Si no}, calcular $\frac{s}{d}$. Si $\frac{s}{d} < \theta$,
			nodo interno como un unico cuerpo.\\Calcular y agregar fuerza.
...
\begin{itemize}
	...
	\item Comenzaremos en la \red{raiz} (nodo interno).
	\begin{itemize}
...
\red{1.} El primer nodo examinado es la raiz.
\begin{itemize}
	\item Cuociente entre A y CM. $\frac{s}{d} = \frac{100}{43.1} > \theta = 0.5$
